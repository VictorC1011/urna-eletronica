    ...
    if(candidato.length > 0) {
        ...
        for(let i in candidato.fotos) {
            if(candidato.fotos[i].small){
                fotosHtml += `<div class="divisao-1-image small"><img src="${candidato.fotos[i].url}" alt="" />${candidato.fotos[i].legenda}</div>`;
            }else{
        ...
        seuVotoPara.style.display = 'block';
        aviso.style.display = 'block';
        descricao.innerHTML = '<dic class="aviso--grande pisca">VOTO NULO</div>'
    }
}
...
function branco(){
    if(numero === '') {
        ...
        alert("Você não pode fazer isso agora. Por favor pressione o botão CORRIGE e em seguida o botão BRANCO.")
    }
}
